\chapter{Procedure}
\section{Setup}
The experiment uses a transparent cryostat, a double-walled piece of glassware \todo{, which does not consist of glass at all :D} that is evacuated during the experiment, shown in \autoref{fig:cryostat}.
...
For each temperature, four measurements are recorded: the current through the sample $I$, the longitudinal voltage $U_\text{L}$ with no external magnetic field and the hall voltage $U_\text{H}^{+,-}$ at a magnetic field of $\pm B = \pm \SI{0.5}{\tesla}$. \todo{Why $\pm$ on both sides?}
