...
\section{Differential Cross-Section}\label{sec:diff_cross_res}\label{a}
\begin{figure}
	\centering
...
The count of every channel is summed up and multiplied with the inverse quantum efficiency function of the detector (as provided in the experiment manual) evaluated at the peak energies to obtain the total number of scattered photons.
...
It is easy to see that the data is in accordance with the model.

\section{Electron Mass Estimation by Compton Energy Shift}\label{b}
\begin{figure}
	\centering
...
\section{Z-Dependence of Compton Scattering}\label{c} %as in https://journals.aps.org/pr/abstract/10.1103/PhysRev.99.59
...
\section{Conclusion}
Different aspects of Compton scattering were tested both qualitatively (\autoref{c}) and quantitatively (\autoref{a} and \autoref{b}).
While in general the experimental data is in good accordance with the models, the absolute values deviate noticeably from the theoretical predictions and in some cases lie outside the calculated error bounds.
This suggests that the experimental setup contains some systematic error, which is not taken into account or corrected in the calculations.

One notable exception is the scattering rate obtained from the lead sample, which deviates by $\approx \SI{40}{\percent}$ from the expected value.
Without a theory to explain this high deviation, this value must be dismissed as an outlier.
