...
def _get_detector_cal():
	def detector_cal_fitfunc(E, a, b, c):
		return a + b * np.exp(-1e-6 * E / c)

	popt, _ = scipy.optimize.curve_fit(
		detector_cal_fitfunc, detector_cal_energy * 1e6, detector_cal_inv_efficiency)
...
if __name__ == '__main__':
	...
	X_fit = np.linspace(0, 1.25e6, 20)
	plt.plot(detector_cal_energy * 1e6, detector_cal_inv_efficiency, 'x')
